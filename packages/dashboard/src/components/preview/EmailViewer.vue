  ...
  <template v-if="emailData">
    ...
    <div class="d-flex align-items-start mb-2 mt-1">
      <div class="w-100">
        ...
        <small class="text-muted">{{ $t('from') }} {{ emailData.from?.name }} &lt;{{ emailData.from?.address }}&gt;</small>
      </div>

    </div>
    <div class="w-100">
        <small class="text-muted">{{ $t('to') }}
    ...
    <h5 class="mb-3" v-if="!emailData.html && !emailData.text">{{ $t('emailEmpty') }}</h5>
    ...
    <template v-if="emailData.attachments?.length > 0">
      <h5 class="mb-3">{{ $t('attachmentsCount', { count: emailData.attachments.length }) }}</h5>
