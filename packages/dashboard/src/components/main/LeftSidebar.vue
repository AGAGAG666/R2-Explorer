<template>
  <div class="q-pa-md" style="height: 100%">
    <div class="flex column" style="height: 100%">
      <q-btn v-if="mainStore.apiReadonly" color="red" stack class="q-mb-lg" :label="$t('readOnly')" />
      <q-btn v-else color="green" icon="add" stack class="q-mb-lg" :label="$t('new')">
        <q-menu>
          <q-list>
      ...
      <q-btn class="q-mb-sm" @click="gotoFiles" color="blue" icon="folder_copy" :label="$t('files')" stack />
      <q-btn v-if="mainStore.config && mainStore.config.emailRouting !== false" class="q-mb-sm" @click="gotoEmail" color="blue" icon="email" :label="$t('email')" stack />
  ...
  <q-dialog v-model="infoPopup" persistent no-route-dismiss>
    <q-card>
      <q-card-section>
        <div class="text-h6">{{ $t('thanks') }}</div>
      </q-card-section>

      <q-card-section class="q-pt-none">
        {{ $t('currentVersion', { version: mainStore.version }) }}<br>
        <template v-if="updateAvailable">
          {{ $t('latestVersion', { version: latestVersion }) }} <a href="https://r2explorer.com/getting-started/updating-your-project/" target="_blank">{{ $t('updateHere') }}</a>.<br>
        </template>
        <br>
        <template v-if="mainStore.auth">
          <b>{{ $t('authentication') }}</b><br>
          {{ $t('method', { method: mainStore.auth.type }) }}<br>
          {{ $t('usernameValue', { username: mainStore.auth.username }) }}
        </template>
        <template v-else>
          {{ $t('notAuthenticated') }}
        </template>
        <br><br>
        <b>{{ $t('serverConfiguration') }}</b><br>
